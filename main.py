...
bot = commands.Bot(
    command_prefix='gg', intents=intents
...
cog_extensions = ('cogs.poll', 'cogs.interacting', 'cogs.error_handler')
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Game("gghelp")
...
async def info(ctx):
    ...
    embed.add_field(
        name="What is it?",
        value=
        "A discord bot made using python for helping you keep track of your goals",
        inline=False)
    embed.add_field(name="Creator",
                    value="Made by \'Hemant\' aka \'LaughingOutClouds\'",
                    inline=False)
    embed.add_field(
        name="Source code",
        value=
...
async def all(ctx):
    ...
    for bot_command in command_list:
        embed.add_field(name=f'{bot_command}',
                        value=f"{bot.command_prefix}{bot_command}",
                        inline=False)
    embed.add_field(name='End',
                    value='We\'ll be coming with new features soon',
                    inline=False)
